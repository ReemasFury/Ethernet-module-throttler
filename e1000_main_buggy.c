...
/* ===== VIDEO THROTTLER POC - START ===== */

static bool e1000_is_video_packet(struct e1000_adapter *adapter,
...
static int e1000_throttler_show(struct seq_file *m, void *v)
{
struct e1000_adapter *adapter = m->private;
seq_printf(m, "Status:    %s\n", adapter->throttle_enabled ? "ENABLED" : "DISABLED");
seq_printf(m, "Port:      %u\n", adapter->throttle_port);
...
static int e1000_throttler_open(struct inode *inode, struct file *file)
{
return single_open(file, e1000_throttler_show, PDE_DATA(inode));
}

static ssize_t e1000_throttler_write(struct file *file,
...
else if (sscanf(buf, "port %u", &value) == 1)
adapter->throttle_port = (u16)value;
else if (sscanf(buf, "delay %u", &value) == 1)
...
adapter->throttle_drop_percent = 0;
...
static const struct file_operations e1000_throttler_fops = {
.owner   = THIS_MODULE,
.open    = e1000_throttler_open,
.read    = seq_read,
.write   = e1000_throttler_write,
.llseek  = seq_lseek,
.release = single_release,
};

static void e1000_create_throttler_proc(struct e1000_adapter *adapter)
{
char proc_name[32];
adapter->throttle_enabled = false;
adapter->throttle_delay_us = 0;
...
static int e1000_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	e1000_create_throttler_proc(adapter);
	cards_found++;
	return 0;
...
static void e1000_remove(struct pci_dev *pdev)
{
	...
	e1000_remove_throttler_proc(adapter);
